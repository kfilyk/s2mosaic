...
Our algorithm compiles several weeks of sentinel2 satellite data into a cloud-free, histogram-corrected multi-band map.
...
We first set out with sentinelHub's built in s2cloudless but realized that those are at only 60m resolution and thus does not yield great results: large swathes of wispy clouds would go undetected. The next process was to download s2Cloudless ourselves and run the cloud detection algorithm locally. This led to a significant improvements in resolution.

S2Cloudless was optimized for when clouds contrasted heavily with their background, such as lush green mountains, red city scape or dark oceans. It was easy for S2Cloudless to create a perfect cloud mask in these best-casescenarios, however it otherwise performed very poorly with the presence of wispy clouds or lighter terrain. Therefore, our implementation changed yet again as we set out with our own method of further refining the cloud mask to minimize wispy clouds through K-means clustering which we originally were using in tandem with canny edge detection to detect the coastline.
...
The image below is an exemplary base tile that was chosen for its low cloud cover.

<img src="/READMImages/l2a_b11.png" width="400"/>

The image below is from a single tile using our composite method to define clouds.
...
Further refinement of our composition method saw us move away from S2Cloudless completely, as there were many instances were simple masking processes performed better at cloud classification. Our final mosaicking process was thus simplified to only using a brightness multiplier of the B01 (coastal aerosol) with the B09 (water vapour) band. These bands correlate higher pixel value to high water vapour reflectance, and thus defined the brightest pixels of an image as having the highest likelihood of belonging a cloud. The next step was to overlay these masks over multiple weeks and swap out high brightness pixels in the final composite with lower brightness pixels from the layered maps. The result was a final composite containing only lowest cloud probability pixels over many weeks of data.
